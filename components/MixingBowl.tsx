...
import { Ingredient, GameStatus } from '../types';
  ...
  return (
    <div
      ...
      className={`relative w-80 h-80 rounded-full border-4 transition-all duration-500 flex flex-col items-center justify-center shadow-[inset_0_0_50px_rgba(0,0,0,0.5)]
        ...
        ${status === GameStatus.MIXING ? 'animate-pulse scale-105 border-amber-500/50' : ''}
        ${status === GameStatus.SUCCESS ? 'border-green-500 shadow-[0_0_30px_rgba(34,197,94,0.3)]' : ''}
        ${isJiggling ? 'jiggle' : ''}
      `}
    >
      ...
      {items.length === 0 ? (
        <div className="text-center text-slate-600 pointer-events-none z-10">
          <div className="w-20 h-20 rounded-full bg-slate-800/50 mx-auto flex items-center justify-center mb-4">
            <i className="fa-solid fa-plus text-2xl opacity-40"></i>
          </div>
          <p className="font-bold text-xs uppercase tracking-widest">Drop Element</p>
        </div>
      ) : (
        <div className="flex gap-4 items-center z-10 px-4">
          ...
          {items.length === 1 && (
            <div className="flex flex-col items-center opacity-40">
              ...
              <span className="text-[10px] font-bold text-slate-600 mt-2 uppercase">Next</span>
            </div>
          )}
        </div>
      )}

      {items.length > 0 && status === GameStatus.IDLE && (
        <button
          onClick={onClear}
      ...
      {status === GameStatus.MIXING && (
        <div className="absolute inset-0 flex flex-col items-center justify-center bg-slate-900/80 rounded-full z-30 backdrop-blur-md">
           ...
           <p className="mt-4 text-xs font-black text-amber-500 uppercase tracking-[0.2em] animate-pulse">Forging...</p>
        </div>
      )}
    </div>
  );
